...
-- Topics: Filtering aggregated results after GROUP BY
...
SELECT week, SUM(present) AS total_staff_present FROM staff_schedule GROUP BY week HAVING SUM(present) < 50;

### DAY 7 Daily Challenge:
...
HAVING SUM(patients_refused) > 100 AND AVG(patient_satisfaction) < 80 ORDER BY total_refused DESC;
